      ...
      module ComparatorParamsTest
        class CreateTest < Minitest::Test
          ...
          def test_process_excludes_extraneous_data
            @data['foo'] = 'foo'
            result = ComparatorParams::Create.process(@data)
            assert_equal %w{kind set_1 set_2 options order linkage_id}.sort, result.keys.sort
          end

          def test_validate_requires_kind_exists
            @data.delete('kind')
            errors = ComparatorParams::Create.validate(@data)
